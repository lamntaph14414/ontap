...
const Login = () => {
    ...
    const validateAll = () => {
        ...
        if (isEmpty(password)) {
            msg.password = "Please input  your passwords"
        }
    ...
    const onSubmitLogin = async () => {
        ...
        try {
            ...
            const res = await axios.post('http://localhost:3500/api/auth/login', params);
            console.log(res);
            if (res.data && res.data.messageCode === 1) {
                // localStorage.getItem(APP_CONSTANTS.USER_TOKEN, res.data.result.access_token)
                setMessage("")
                navigate("/")
            }else{
                    ...
                    <div className="w-full lg:w-2/3 px-6 py-16">
                        ...
                        <form>
                            <div className="mb-4">
                                ...
                                />
                                <p className="text-red-400 text-danger italic ">{validationMsg.email}</p>
                            </div>
                            <div className="mb-4">
                                ...
                                />
                                <p className="text-red-400 italic text-danger">{validationMsg.password}</p>
                            </div>
